        ...
        <div className="rounded-xl border border-border/40 bg-card">
          <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="title" placeholder="e.g., Premium Bot License" {...register('title', { required: 'Title is required' })} className="bg-muted/30 border-border/40" />
                  {errors.title && <p className="text-xs text-destructive">{errors.title.message}</p>}
                </div>
                <div className="space-y-2">
                  <Label htmlFor="slug">Slug</Label>
                  <Input id="slug" {...register('slug')} className="bg-muted/30 border-border/40 font-mono text-sm" />
                </div>
              </div>

              <div className="space-y-2">
                <Label htmlFor="shortDescription">Short Description <span className="text-destructive">*</span></Label>
                <Textarea id="shortDescription" placeholder="Brief description for cards..." rows={2} {...register('shortDescription', { required: 'Required' })} className="resize-none bg-muted/30 border-border/40" />
                {errors.shortDescription && <p className="text-xs text-destructive">{errors.shortDescription.message}</p>}
              </div>

              <div className="space-y-2">
                <Label htmlFor="longDescription">Long Description</Label>
                <Textarea id="longDescription" placeholder="Detailed description..." rows={4} {...register('longDescription')} className="resize-none bg-muted/30 border-border/40" />
              </div>
            </div>
                ...
                {iconOptions.map(({ name, Icon }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] leading-tight font-medium truncate w-full text-center">{name}</span>
                    </button>
                  )
                })}
              </div>
            ...
            <div className="space-y-4">
              ...
              <Textarea placeholder="Features separated by commas..." rows={3} {...register('features')} className="resize-none bg-muted/30 border-border/40" />
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 max-w-md">
                <button type="button" onClick={() => handlePricingTypeChange('subscription')} className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-medium transition-all ${pricingType === 'subscription' ? 'border-primary bg-primary/10 text-primary shadow-sm' : 'border-border/40 bg-muted/30 text-muted-foreground hover:border-border hover:bg-muted/50'}`}>
                  <Repeat className="h-4 w-4" /> Recurring Subscription
                </button>
                <button type="button" onClick={() => handlePricingTypeChange('one_time')} className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-medium transition-all ${pricingType === 'one_time' ? 'border-primary bg-primary/10 text-primary shadow-sm' : 'border-border/40 bg-muted/30 text-muted-foreground hover:border-border hover:bg-muted/50'}`}>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={addTier} className="h-7 text-xs gap-1.5">
                    <Plus className="h-3 w-3" /> Add Tier
                  </Button>
                </div>
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
                  {pricingTiers.map((tier, index) => (
                    <TierEditor key={index} tier={tier} index={index} pricingType={pricingType} totalTiers={pricingTiers.length} onUpdate={updateTier} onRemove={removeTier} />
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="sortOrder" type="number" min="0" placeholder="0" {...register('sortOrder', { min: { value: 0, message: 'Must be >= 0' } })} className="bg-muted/30 border-border/40 tabular-nums" />
                </div>
                <div className="flex items-center gap-3 pt-6">
                  <Switch checked={watch('isActive')} onCheckedChange={(checked) => setValue('isActive', checked)} id="isActive" />
            ...
            <div className="flex items-center justify-end gap-3 pt-6 border-t border-border/40">
              <Button type="button" variant="outline" asChild disabled={submitting} className="rounded-xl">
                <Link href="/admin/services">Cancel</Link>
              </Button>
              <Button type="submit" disabled={submitting} className="rounded-xl">
                {submitting ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin" />Creating...</>) : (<><Save className="h-4 w-4 mr-2" />Create Service</>)}
              </Button>
            </div>
          </form>
        </div>
